...
const sampleListing = {
  ...
  business_deposit_account_features: [
    { category: 'fraud_protection', description: 'Instant card freeze', value: null, sort_order: 2 },
    { category: 'cash_handling', description: 'Free cash deposits up to $2,000/mo', value: 2000, sort_order: 1 },
  ],
}
...
describe('get_business_checking_listing handler', () => {
  ...
  it('returns the matching listing with additional_fees and features', async () => {
    ...
    // column and must not also appear in additional_fees (Decision 3).
    expect(item.additional_fees).toHaveLength(1)
    expect(item.additional_fees[0].fee_type).toBe('overdraft')
    expect(item.additional_fees[0].amount).toBe(35)
    expect(item.additional_fees.some((f: { fee_type: string }) => f.fee_type === 'wire_domestic_outgoing')).toBe(
      false
    )

    // sorted by sort_order
    expect(item.features).toHaveLength(2)
    expect(item.features[0].description).toBe('Free cash deposits up to $2,000/mo')
    expect(item.features[1].description).toBe('Instant card freeze')
  })
