...
export async function handleQueryBusinessCd(
  ...
  let query = supabase
    ...
    .select('*, business_cd_details(*), institutions(name)')
    .eq('listing_status', 'active')
    .eq('product_type', 'cd')
    .order('apy', { ascending: false, foreignTable: 'business_cd_details' })

  if (args.apy_min !== undefined) {
    query = query.gte('business_cd_details.apy', args.apy_min)
  }
  ...
  if (args.term_months_min !== undefined) {
    query = query.gte('business_cd_details.term_months', args.term_months_min)
  }
  if (args.term_months_max !== undefined) {
    query = query.lte('business_cd_details.term_months', args.term_months_max)
  }
  ...
  if (args.auto_renew !== undefined) {
    ...
    results = results.filter((row) => {
      const details = row.business_cd_details as Record<string, unknown> | null
